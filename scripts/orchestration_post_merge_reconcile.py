...
def _ensure_production_work_dispatch(manifest):
    ...
    instructions = (
        ...
        "Use any supported interactive browser mechanism to render and operate the exact Production "
        "deployment_url; the mechanism need not be named Cloud Browser. Perform the applicable input, "
        "click/tap, iframe, and browser-history interactions. Search/fetch/source/DOM inspection, "
        "screenshots alone, CI/status, and Preview evidence cannot substitute; if no qualifying "
        "interactive browser is available, return BLOCKED with the journey UNTESTED. Write exactly one "
        "gameai-acceptance/v1 candidate to PR #%(pr)s for this "
