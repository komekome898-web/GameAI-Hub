...
External bridges start fail-closed: Codex dispatch/ack/bot/resume, Work trigger/writeback/model/attestation, and Preview/Production readiness are **NEEDS EXPERIMENT**, **DISABLED_UNVERIFIED**, or **UNCONFIGURED** until observed. The repository creates a fenced Codex outbox claim after Preview FAIL, but no external dispatch is claimed until ACK correlation is proven. Comment creation is not acknowledgement. See `WORK_ACCEPTANCE.md` for the experiment matrix and setup.
