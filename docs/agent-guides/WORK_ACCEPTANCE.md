...
Preview dispatch is the creation of exactly one fenced `gameai-work-dispatch/v1` PR comment for a runnable claim. That marker means only `contract_emitted`, never execution started. No uniquely attributable native Work ACK channel is currently supported in this repository: the shared connector identity is insufficient and standalone ACK ingress remains disabled. A fully trusted exact-claim result or capability signal may atomically prove execution and result receipt; `execution_acknowledged` is reserved for a future uniquely attestable channel. With neither ACK nor result, the watchdog records `timed_out` and blocks visibly. Configure the native task to react to that PR-comment activity, not to infer readiness from CI or labels. It must re-fetch the Issue Manifest, PR head, and same-claim candidates before opening the contract's exact deployment URL. Duplicate/stale events are no-ops. Scheduled reconciliation recreates a missing dispatch comment after a crash, but never creates a second same-claim dispatch. The connector-authored candidate ingress independently re-fetches the comment and requires observed GitHub App ID `1144995`; it rejects zero/multiple current-claim candidates and then delegates the verdict to the existing reducer. Connector writeback alone records `CONFIGURED_UNVERIFIED`, not `VERIFIED`; only separately observed task start plus browser evidence may justify `VERIFIED`.
...
Production begins with the capability handshake in `orchestration/production-acceptance.md`. An unavailable interactive browser is a trusted, fenced technical-retry signal—not a product/UI verdict. The ingress automatically creates and dispatches a fresh Production claim for the unchanged merged release/deployment, bounded to three retries. A claim-scoped model fallback expires with that claim. Production PASS always requires real interactive-browser evidence; Preview, CI, HTTP, search, source/DOM inspection, and deployment status never substitute.
