...
export default async function ToolPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <article className="detail">
    ...
    <section className="verdict"><p className="eyebrow">WORKFLOW FIT</p><h2>{s.conclusion}</h2><p>以下は掲載情報と公式資料の確認状態に基づく判断材料です。プロジェクト全体の構成はBuilderで確認してください。</p></section>
    ...
    <section><p className="section-label">VERIFIED USES</p><h2>制作工程での用途</h2><div className="tags">{s.primaryUses.map(x=><span key={x}>{x}</span>)}</div><p className="tool-builder-link"><Link className="button ghost" href="/builder">自分の条件で開発構成を検討する</Link></p></section>
    ...
    <section><p className="section-label">ALTERNATIVES</p><h2>代替候補</h2>{alternatives.length?<><div className="alternative-list">{alternatives.map(a=><Link key={a.id} href={`/tools/${a.slug}`}><strong>{a.name}</strong><span>{a.summary}</span><b>詳細 →</b></Link>)}</div><Link className="button ghost" href={`/compare?ids=${[s.slug,...alternatives.map(a=>a.slug)].slice(0,4).join(',')}`}>代替候補と比較する</Link></>:<p>現在、同一用途で十分に検証できた比較候補はありません。Builderで制作条件を指定すると、手動確認が必要な候補も区別して確認できます。</p>}</section>
