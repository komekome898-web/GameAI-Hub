...
export function track(name:EventName,properties:EventProperties={}){if(typeof window==='undefined')return;window.dispatchEvent(new CustomEvent('gameai:event',{detail:{name,properties}}));if(process.env.NODE_ENV==='production'){const gtag=(window as typeof window & {gtag?:Gtag}).gtag;if(gtag)gtag('event',name,properties);}else console.debug('[GameAI analytics]',name,properties);}
