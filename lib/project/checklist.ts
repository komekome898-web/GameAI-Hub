...
export function buildChecklist(plan:ProjectPlan):BuildChecklistStep[] {
  ...
  const items:BuildChecklistStep[]=[
    ...
    step(plan,'core-loop','コアループを通す',plan.verticalSlice[0]?.title??'最小のゲームループが動く','面白さと最大の技術リスクを先に検証するため。',code,plan.today,code?.doneWhen??['最小ループを操作して確認できる']),
  ...
  if(plan.brief.capabilities.includes('art-2d'))optional('assets-2d','代表2D素材を組み込む','2D素材1点がゲーム内で正しく表示される','透過・サイズ・可読性を量産前に確認するため。','visuals',['代表画像1点の仕様を書く','ツールで生成・出力する','ゲームへimportして実機検品する']);
  ...
  if(plan.brief.capabilities.includes('voice'))optional('voice','代表台詞を組み込む','台詞ID付き音声がゲーム内で再生される','同意・発音・音量を量産前に確認するため。','voice',['話者同意と台詞IDを確認する','代表台詞3件を生成・出力する','IDで組み込み発音・音量を確認する']);
