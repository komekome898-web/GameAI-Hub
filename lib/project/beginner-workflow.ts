...
export function beginnerWorkflowSteps(plan: ProjectPlan): BuildChecklistStep[] {
  ...
  const steps = [
    make('core-loop', novel ? '台詞が進むゲームを動かす' : battle ? '1対1のバトルを動かす' : tapScore ? `${intent.actor ?? '対象'}のタップで得点が増えるゲームを作る` : movementGoal ? '動かしてゴールするゲームを作る' : '確認した主要操作を1つ動かす',
      novel ? '背景と人物を見ながら、台詞を最後まで読める試作' : battle ? '攻撃・HP・勝敗・やり直しが動く1対1の試作' : tapScore ? `${intent.actor ?? '対象'}をタップすると得点が1増え、やり直せる試作` : movementGoal ? 'タップで移動し、ゴールしてやり直せる試作' : '確認した操作・結果・やり直しだけが動く試作', scope,
    ...
    make('ui-prototype', '開始から終わりまでをつなぐ', novel ? '「はじめる」→会話→おしまい→最初からを操作できるゲーム' : '「はじめる」→プレイ→結果→もう一度を操作できるゲーム', '遊ぶ人が開始方法と終わった状態を見分けられるようにする。', update,
      novel ? '冒頭にタイトルと「はじめる」を追加し、会話の終わりに「おしまい」と「最初から」を表示する。背景・人物・台詞・次へ操作と言語切り替えは残す。未指定のサブジャンルや選択肢は追加しない。' : `冒頭にタイトルと「はじめる」を追加する。開始前はゲームが進まない。結果には${tapScore ? '現在の得点' : battle ? '勝敗' : movementGoal ? 'クリア' : '操作結果'}を文字で示し、「もう一度」で開始画面へ戻る。現在の操作を残す。`,
      ['「はじめる」を押してからゲームが始まる', novel ? '最後の台詞の後に「おしまい」が表示される' : tapScore ? 'タップ／クリック後の得点が文字で表示される' : battle ? '最後まで遊ぶと勝敗が文字で表示される' : movementGoal ? '最後まで遊ぶとクリアが文字で表示される' : '主要操作の結果が文字で表示される', 'やり直すと開始画面に戻り、もう一度最後まで遊べる']),
  ...
  steps.push(
    make('qa', '最初から最後まで遊んで確かめる', '開始・操作・終わり・やり直しを自分で確認した試作', '完了にする前に、作った本人が実際に遊べることを確かめる。',
      ['「ゲームを表示」で最初から始め、画面内のボタンだけで最後まで遊ぶ。', battle ? '勝った場合と負けた場合の表示を確認する。一方を確認できなければ「ここで詰まった」から、その状態を試す方法をAIに聞く。' : novel ? '台詞を順番に読み、最後まで進めることを確認する。' : tapScore ? `${intent.actor ?? '対象'}を複数回タップ／クリックし、その回数だけ得点が増えることを確認する。` : movementGoal ? '画面の端まで移動しても操作でき、ゴールへ到達できることを確認する。' : '確認済みの主要操作を行い、結果表示を確認する。', 'やり直して、もう一度遊ぶ。スマホでは縦向きで文字とボタンを確認する。', '困った点があれば「ここで詰まった」で具体的な操作を追記し、現在のコードと一緒に同じAIへ送る。変更前に保存してから直し、もう一度試す。'], '',
