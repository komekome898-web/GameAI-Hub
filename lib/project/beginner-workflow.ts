...
import { projectIntentProfile } from './intent-profile';
...
export function beginnerWorkflowSteps(plan: ProjectPlan): BuildChecklistStep[] {
  ...
  const context = plan.brief.details.length
    ...
    : '追加の固有設定は未指定';
  const genre = intent.label;
  const scope = battle
    ? '今回は味方1体と敵1体のバトルだけ。収集・育成・図鑑は次の試作に残す。'
    : novel ? '今回は背景と人物の仮表示、短い会話1場面だけ。未指定のサブジャンル、選択肢や分岐は追加しない。'
      : tapScore ? `今回は1画面に${intent.actor ?? '対象'}を1つ表示し、タップ／クリックで得点が1増えるところだけ。移動やゴールは追加しない。`
        : movementGoal ? '今回は1画面で移動してゴールするところだけ。ステージや敵を増やさない。'
          : '確認済みの主要操作を1つだけ実装する。未確認の操作、目的、ジャンルは追加しない。';
  ...
  const coreRequest = novel
    ? `背景の色面と人物の仮の姿、話者名、日本語の短い台詞3つ、「次へ」を作る。最後は「この場面はおしまい」と表示し「最初から」で最初の台詞へ戻す。タップとクリックで操作できるようにする。${plan.brief.locale === 'ja-en' || plan.brief.capabilities.includes('localization') ? '「日本語 / English」切り替えを置き、同じ3つの台詞を日英で切り替える。言語を変えても同じ台詞番号を保つ。' : ''}`
    : battle ? `味方モンスター1体と敵1体、それぞれの名前とHP、「攻撃」ボタンを作る。攻撃したら敵も反撃する。HPは0未満にならず、敵HPが0なら勝利を文字表示し、終了後は攻撃を止める。「もう一度」で初期HPへ戻す。${explicitDefeat ? '味方HPが0なら敗北を文字表示し、勝利と敗北の両方を確認できるようにする。' : '敗北条件は追加しない。'}`
      : tapScore ? `${intent.actor ?? '対象'}を1つ図形または文字で表示し、タップ／クリックするたびに画面の得点を1増やす。初期値は0。「もう一度」で得点を0へ戻す。移動、ゴール、収集は追加しない。`
        : movementGoal ? 'プレイヤーとゴールを図形で作る。矢印キー・WASDに加え、画面内の上下左右ボタンのタップでも移動する。ゴールへ触れたら「クリア！」を表示し「もう一度」で最初の位置へ戻す。'
          : '確認済みのゲーム情報にある主要操作を1つだけ実装し、その操作結果を文字で表示する。「もう一度」で初期状態へ戻す。確認済みでない移動、ゴール、戦闘、収集、恋愛要素は追加しない。';
  const coreCriteria = novel
    ? ['背景・人物・話者名・最初の台詞が見える', '「次へ」で台詞が順番に変わり、最後に終わりの表示が出る', ...(plan.brief.locale === 'ja-en' || plan.brief.capabilities.includes('localization') ? ['日本語と英語を切り替えても同じ台詞番号が表示される'] : []), '「最初から」で最初の台詞に戻る']
    : battle ? ['味方と敵の名前・HPが見える', '「攻撃」で双方のHPが変わり、敵HPが0になると勝利が文字表示される', ...(explicitDefeat ? ['味方HPが0になると敗北が文字表示される'] : []), '終了後は攻撃できず、「もう一度」で初期HPへ戻る']
      : tapScore ? [`${intent.actor ?? '対象'}と得点0が見える`, `${intent.actor ?? '対象'}をタップ／クリックするたびに得点が1増える`, '「もう一度」で得点が0へ戻る']
        : movementGoal ? ['プレイヤーとゴールが見える', '画面内の上下左右ボタンで移動できる（キーボードがある場合は矢印キーでも確認）', 'ゴールで「クリア！」が出て、「もう一度」で最初の位置へ戻る']
          : ['確認済みの主要操作を実行できる', '操作結果が文字で分かる', '「もう一度」で初期状態へ戻る'];
  const steps = [
    make('core-loop', novel ? '台詞が進むゲームを動かす' : battle ? '1対1のバトルを動かす' : tapScore ? `${intent.actor ?? '対象'}のタップで得点が増えるゲームを作る` : movementGoal ? '動かしてゴールするゲームを作る' : '確認した主要操作を1つ動かす',
      novel ? '背景と人物を見ながら、台詞を最後まで読める試作' : battle ? `攻撃・HP・${explicitDefeat ? '勝敗' : '勝利結果'}・やり直しが動く1対1の試作` : tapScore ? `${intent.actor ?? '対象'}をタップすると得点が1増え、やり直せる試作` : movementGoal ? 'タップで移動し、ゴールしてやり直せる試作' : '確認した操作・結果・やり直しだけが動く試作', scope,
    ...
    make('ui-prototype', '開始から終わりまでをつなぐ', novel ? '「はじめる」→会話→おしまい→最初からを操作できるゲーム' : '「はじめる」→プレイ→結果→もう一度を操作できるゲーム', '遊ぶ人が開始方法と終わった状態を見分けられるようにする。', update,
      novel ? '冒頭にタイトルと「はじめる」を追加し、会話の終わりに「おしまい」と「最初から」を表示する。背景・人物・台詞・次へ操作と言語切り替えは残す。未指定のサブジャンルや選択肢は追加しない。' : `冒頭にタイトルと「はじめる」を追加する。開始前はゲームが進まない。結果には${tapScore ? '現在の得点' : battle ? (explicitDefeat ? '勝敗' : '勝利結果') : movementGoal ? 'クリア' : '操作結果'}を文字で示し、「もう一度」で開始画面へ戻る。現在の操作を残す。`,
      ['「はじめる」を押してからゲームが始まる', novel ? '最後の台詞の後に「おしまい」が表示される' : tapScore ? 'タップ／クリック後の得点が文字で表示される' : battle ? `最後まで遊ぶと${explicitDefeat ? '勝敗' : '勝利結果'}が文字で表示される` : movementGoal ? '最後まで遊ぶとクリアが文字で表示される' : '主要操作の結果が文字で表示される', 'やり直すと開始画面に戻り、もう一度最後まで遊べる']),
  ...
  steps.push(
    make('qa', '最初から最後まで遊んで確かめる', '開始・操作・終わり・やり直しを自分で確認した試作', '完了にする前に、作った本人が実際に遊べることを確かめる。',
      ['「ゲームを表示」で最初から始め、画面内のボタンだけで最後まで遊ぶ。', battle ? (explicitDefeat ? '勝った場合と負けた場合の表示を確認する。一方を確認できなければ「ここで詰まった」から、その状態を試す方法をAIに聞く。' : '敵HPを0にして勝利結果を確認する。敗北条件は未指定なので要求しない。') : novel ? '台詞を順番に読み、最後まで進めることを確認する。' : tapScore ? `${intent.actor ?? '対象'}を複数回タップ／クリックし、その回数だけ得点が増えることを確認する。` : movementGoal ? '画面の端まで移動しても操作でき、ゴールへ到達できることを確認する。' : '確認済みの主要操作を行い、結果表示を確認する。', 'やり直して、もう一度遊ぶ。スマホでは縦向きで文字とボタンを確認する。', '困った点があれば「ここで詰まった」で具体的な操作を追記し、現在のコードと一緒に同じAIへ送る。変更前に保存してから直し、もう一度試す。'], '',
      [...coreCriteria, ...(battle && explicitDefeat ? ['勝ちと負けの両方で終了とやり直しを確認した'] : []), '文字が読め、操作ボタンを押せる', 'もう一度最初から最後まで遊べる'], []),
