...
export function ArticleFrame({
  ...
  return (
    <article className="page-shell article-shell">
      <ArticleAnalytics slug={article.slug} />
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: serializeJsonLd(articleJson) }}
      />
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: serializeJsonLd(breadcrumbs) }}
      />
      <ArticleReadingGuide article={article} />
      ...
      <section className="sources">
        ...
        {article.sources.length ? (
          <ul>
            {article.sources.map((source) => (
              <li key={source.url}>
                ...
                <span>
                  {source.kind === "primary" ? "一次資料" : "実体験"} · 確認{" "}
                  {source.verifiedAt}
                </span>
              </li>
            ))}
          ...
          <p>
            この記事は運営者の実体験・意見を扱い、外部製品の仕様を根拠とする記事ではありません。
          </p>
        )}
        <p>
          仕様・料金・規約を扱う場合は公開前と更新時に一次資料を確認し、確認できない情報は不明と記載します。更新基準は
