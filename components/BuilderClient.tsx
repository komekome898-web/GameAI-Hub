...
function BuilderResult({input,onEdit,focusRef,assumptionSource}:{input:ProjectInput;onEdit:()=>void;focusRef:RefObject<HTMLHeadingElement|null>;assumptionSource:string}) {
  ...
  return <div className="result-shell">
    ...
    <section id="stage-decisions"><p className="section-label">STAGE DECISIONS</p><h2>工程ごとの判断と次の行動</h2><div className="stage-results">{active.map((stage,i)=><details key={stage.stage} className="stage-decision" open={i===0}><summary><span>STEP {i+1} · {stage.requirement==='required'?'必須':'任意'}</span><strong>{labels[stage.stage]}</strong><small>{stage.primary?.service.name??'手動工程'}</small></summary><div className="stage-decision-body"><p>{productionStages.find(x=>x.id===stage.stage)?.objective}</p>{stage.primary?<><div className="recommended-tool"><small>条件一致候補</small><h4><Link href={`/tools/${stage.primary.service.slug}`}>{stage.primary.service.name}</Link></h4><p className="candidate-verification">最終確認日: {stage.primary.service.lastVerified} · {stage.primary.service.sources.slice(0,2).map((source,index)=><span key={source.url}>{index?' / ':''}<a href={source.url} target="_blank" rel="noopener">公式{source.type==='pricing'?'料金':'情報'} ↗</a></span>)}</p><p><strong>選定理由:</strong> {stage.primary.reason}</p></div><ResultFacts tool={stage.primary}/>{stage.alternatives[0]&&<div className="alternative"><strong>代替案: <Link href={`/tools/${stage.alternatives[0].service.slug}`}>{stage.alternatives[0].service.name}</Link></strong><p>{stage.alternatives[0].reason}</p><Link href={`/compare?ids=${stage.primary.service.slug},${stage.alternatives[0].service.slug}&stage=${stage.stage}`}>2候補を比較する →</Link></div>}<OutboundLink service={stage.primary.service} page="builder-result" placement={`stage-${stage.stage}`}/></>:<div className="manual-stage"><strong>手動工程</strong><p>{stage.manualFallback}</p></div>}{stage.reviewCandidates.map(candidate=><div className="alternative" key={candidate.service.slug}><strong>要手動確認（推薦ではありません）: <Link href={`/tools/${candidate.service.slug}`}>{candidate.service.name}</Link></strong><CandidateVerification service={candidate.service}/><p>{candidate.reason}</p><p>{candidate.manualChecks.join(' / ')}</p></div>)}<dl className="decision-facts"><div><dt>成果物</dt><dd>{stage.deliverable}</dd></div><div><dt>完了条件</dt><dd>{stage.acceptanceCriteria.join(' / ')}</dd></div><div><dt>次工程への受け渡し</dt><dd>{stage.handoff}</dd></div><div><dt>人が行う作業</dt><dd>{stage.manualTasks.join(' / ')}</dd></div></dl><div className="stage-next"><strong>次の行動</strong><p>{stage.nextAction}</p></div></div></details>)}</div></section>
...
function ResultFacts({tool}:{tool:ReturnType<typeof recommendProject>['stages'][number]['primary'] extends infer T?NonNullable<T>:never}) { return <dl className="decision-facts"><div><dt>根拠</dt><dd>{tool.evidence.length?tool.evidence.join(' / '):'登録済み用途との一致'}</dd></div><div><dt>コスト表示</dt><dd>{tool.costVisibility}</dd></div><div><dt>既知の制約</dt><dd>{tool.limitations.join(' / ')||'掲載済みの制約なし'}</dd></div><div><dt>不明・要確認</dt><dd>{[...tool.unknowns,...tool.manualChecks].join(' / ')||'追加確認なし'}</dd></div></dl> }
