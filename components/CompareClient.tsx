...
function CompareClientState({services,initial,params}:{services:Service[];initial?:string[];params:Readonly<URLSearchParams>}){
  ...
  const stageMatches=validStage?selected.filter(service=>compareStageCategories[validStage].includes(service.category)):[];
  return <>
    {validStage&&<div className="compare-context" role="status"><strong>Projectの「{phaseLabelsForCompare(validStage)}」工程から比較しています。</strong><a href="/project">Projectへ戻る →</a></div>}
    ...
    {selected.length>=2&&<section className="compare-decision" aria-labelledby="compare-decision-title"><span>最初に判断</span><h2 id="compare-decision-title">{validStage?`「${phaseLabelsForCompare(validStage)}」工程との用途一致`:'工程を指定すると、用途との一致を確認できます'}</h2>{validStage&&stageMatches.length?<p>登録された用途カテゴリが一致する候補：<strong>{stageMatches.map(service=>service.name).join('、')}</strong>。これは品質順位ではありません。</p>:<p>選択中の情報だけでは優先順位を確定できません。下の差分と公式確認事項で判断してください。</p>}<ul>{selected.map(service=><li key={service.id}><strong>{service.name}</strong> — {service.primaryUses.join('、')}。注意：{service.weaknesses[0]}</li>)}</ul><p>価格、権利、品質を推測して勝者を決めません。</p></section>}
...
const phaseLabelsForCompare=(stage:string)=>({code:'コード',voice:'声・音声','3d':'3D素材',visuals:'2Dビジュアル',animation:'アニメーション','music-sfx':'BGM・SFX','npc-dialogue':'NPC会話',localization:'翻訳',testing:'テスト',publishing:'公開素材'}[stage]??stage);
const compareStageCategories:Record<string,string[]>={code:['coding-agent','ide-ai','general-llm'],voice:['voice'],'3d':['3d-model','texture-material','rigging'],visuals:['2d-image','concept-art','character-consistency'],animation:['animation','rigging'],'music-sfx':['music','sfx'],'npc-dialogue':['npc-dialogue'],localization:['localization'],testing:['qa-testing'],publishing:['marketing-store-assets','video-trailer']};
