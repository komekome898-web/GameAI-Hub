...
export function ToolsExplorer({services,initialCategory='すべて'}:{services:Service[];initialCategory?:string}){
  ...
  const categories=['すべて',...new Set(services.map(s=>s.category))];
  ...
  return <>
    <div className="filter-panel">
      <div className="filter-copy"><span>FILTER</span><strong>条件を絞り込む</strong></div>
    ...
    <div className="results-head"><p aria-live="polite"><strong>{shown.length}</strong> 件のツール</p><span>料金・条件は公式情報で最終確認してください</span></div>
    ...
    {!shown.length&&<div className="no-results"><strong>該当するツールがありません</strong><p>キーワードまたはカテゴリを変更してください。</p></div>}
  </>
}
