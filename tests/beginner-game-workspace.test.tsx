...
import {
  ...
  makeGamePreview,
} from "@/components/BeginnerGameWorkspace";

const game =
  "<!doctype html><html><body><button>はじめる</button><script>window.gameStarted=true;</script></body></html>";
...
describe("beginner game isolation and recovery", () => {
  it("places supplied HTML only in an opaque sandbox after an explicit action", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByLabelText("ゲームのコード")).toBeTruthy(),
    );
    fireEvent.change(screen.getByLabelText("ゲームのコード"), {
      target: { value: game },
    });
  ...
  it("puts restrictive CSP before attacker scripts and removes refresh/base/embedded frames", () => {
    const html = makeGamePreview(
      '<meta http-equiv="refresh" content="0;url=https://example.test"><meta http-equiv="Content-Security-Policy" content="default-src *"><base href="https://example.test"><iframe src="https://example.test"></iframe><script>fetch("https://example.test")</script>',
    );
  ...
  it("restores saved source without running it and keeps projects separate", async () => {
    localStorage.setItem(
      "gameai:beginner-game:v1:a",
      JSON.stringify({ code: game, working: game }),
    );
    const { rerender } = render(<BeginnerGameWorkspace projectId="a" />);
    await waitFor(() =>
      expect(
        (screen.getByLabelText("ゲームのコード") as HTMLTextAreaElement).value,
      ).toBe(game),
    );
    expect(screen.queryByTitle("作ったゲームの動作確認")).toBeNull();
    ...
    expect(localStorage.getItem("gameai:beginner-game:v1:a")).toContain(
      "はじめる",
    );
  });
  ...
  it("keeps the user-confirmed working version when a later game is previewed", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "ゲームを表示" }));
    fireEvent.click(
      screen.getByRole("button", { name: "この版は動いたと記録" }),
    );
    fireEvent.change(screen.getByLabelText("ゲームのコード"), {
      target: { value: "<html><body>変更後</body></html>" },
    });
    fireEvent.click(screen.getByRole("button", { name: "ゲームを表示" }));
    ...
    expect(
      screen.getByTitle("作ったゲームの動作確認").getAttribute("srcdoc"),
    ).toContain("はじめる");
  });
  ...
  it("loads a saved HTML file without executing it until the preview action", async () => {
    ...
    fireEvent.change(screen.getByLabelText("保存したゲームを開く"), {
      target: { files: [file] },
    });
    await waitFor(() =>
      expect(
  ...
  it("accepts only the active iframe runtime-error schema and renders text safely", async () => {
    render(<BeginnerGameWorkspace projectId="runtime" initialCode={game} />);
    await waitFor(() =>
    ...
    window.dispatchEvent(
      new MessageEvent("message", {
        ...
        data: {
          type: "gameai-preview-error",
          channel,
          kind: "error",
          message: "QA_RUNTIME_0904",
          line: 9,
        },
      }),
    );
    expect(
      await screen.findByText("ゲーム内でエラーが発生しました"),
    ).toBeTruthy();
    expect(screen.getByText("QA_RUNTIME_0904")).toBeTruthy();
    window.dispatchEvent(
      new MessageEvent("message", {
        source: window,
        data: {
